...
editSeriesForm?.addEventListener('submit', async (e) => {
  ...
  if (slug) {
    if (isReservedSlug(slug)) {
      setMsg(esMsg, `"${slug}" is a reserved system route and cannot be used.`, true);
      return;
    }
    const slugLower = slug.toLowerCase();
    const taken = Object.entries(catalogSeriesMap || {}).some(
      ([id, s]) => id !== seriesId && (s?.slug || '').toLowerCase() === slugLower
    );
    if (taken) {
      setMsg(esMsg, `The URL slug "${slug}" is already in use by another publication.`, true);
      return;
    }
  }
...
editEditionForm?.addEventListener('submit', async (e) => {
  ...
  if (slug) {
    if (isReservedSlug(slug)) {
      setMsg(eeMsg, `"${slug}" is a reserved system route and cannot be used.`, true);
      return;
    }
    const slugLower = slug.toLowerCase();
    const edRow = openOrgEditions.find((e) => e.id === editionId);
    const seriesId = edRow?.series_id || pubsBrowseSeriesId;
    const collision = openOrgEditions.some(
      (e) => e.id !== editionId && (seriesId ? e.series_id === seriesId : true) && (e.slug || '').toLowerCase() === slugLower
    );
    if (collision) {
      setMsg(eeMsg, `An edition with the URL slug "${slug}" already exists in this publication. Edition slugs must be unique within the publication.`, true);
      return;
    }
  }
