...
let embedWrapperResizeObserver = null;

/**
 * True when the public edition route is framed (rsamdio.org embed).
 * @returns {boolean}
 */
function isReaderEmbedMode() {
  if (typeof document === 'undefined') return false;
  if (document.documentElement?.dataset?.readerEmbed === 'true') return true;
  try {
 ...
 * Standalone: layout viewport (window). Embed: flipbook wrapper box (iframe size).
 * Never visualViewport — keyboard/pinch must not flip mode.
 * @returns {boolean}
 */
function shouldUseSpreadLayout() {
  ...
  if (isReaderEmbedMode()) {
    const wrapper = document.getElementById('flipbook-wrapper');
    const shell = document.querySelector('.reader-route-shell');
    const box = wrapper || (shell instanceof HTMLElement ? shell : null);
...
function lockReaderPageScroll(lock) {
  ...
  // Embed: fill the iframe via CSS data-reader-embed; do not apply full-tab scroll lock.
  if (lock && isReaderEmbedMode()) {
...
function showReaderView() {
  getReaderElements();
  // Keep embed attrs in sync after chrome HTML inject (page mode).
  if (isReaderEmbedMode()) {
    const html = document.documentElement;
    html.dataset.readerEmbed = 'true';
    const shell = document.querySelector('.reader-route-shell');
    if (shell instanceof HTMLElement) shell.dataset.readerEmbed = 'true';
    if (readerView) readerView.dataset.readerEmbed = 'true';
  }
  ...
  ensureEmbedWrapperResizeObserver();
  if (readerLoading) {
...
function hideReaderView() {
  ...
  if (embedWrapperResizeObserver) {
    try {
      embedWrapperResizeObserver.disconnect();
    } catch (_) {}
    embedWrapperResizeObserver = null;
  }
...
function ensureEmbedWrapperResizeObserver() {
  if (typeof ResizeObserver === 'undefined') return;
  if (!isReaderEmbedMode()) return;
  const wrapper = document.getElementById('flipbook-wrapper');
  if (!wrapper) return;
  if (embedWrapperResizeObserver) {
    try {
  ...
  embedWrapperResizeObserver = new ResizeObserver(() => {
    scheduleLayoutRelayoutCheck();
  });
  embedWrapperResizeObserver.observe(wrapper);
}
