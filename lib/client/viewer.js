...
export function openReader(publication) {
  ...
  return enqueueReaderOp(async () => {
    try {
      ...
      // Start non-critical assets in background to unblock PDF parsing
      ensurePdfViewerCss().catch(() => {});
      ensurePageFlipCss().catch(() => {});
      const pageFlipTask = ensurePageFlip();
      ...
      activePdfDoc = pdfDoc;
      await pageFlipTask; // Wait for PageFlip library before building the flipbook UI
      if (myLoad !== loadGeneration) return;
