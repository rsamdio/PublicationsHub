...
function setReaderLocationHash(publication) {
  ...
  // Standalone edition page already owns `/p/.../e/...` — do not rewrite.
  if (isReaderPageMode()) return;
  // Public App Router paths: /p/[seriesId]/e/[editionId]
  if (typeof location !== 'undefined' && location.pathname.startsWith('/p/')) {
    const sid =
      publication._seriesCanonicalId ||
      getSeriesCanonicalIdForPublication(publication) ||
      publication.id;
    const next = editionPath(String(sid), String(publication.id));
...
function clearReaderLocationHash() {
  ...
  if (typeof location !== 'undefined' && location.pathname.startsWith('/p/')) {
    ...
    const pubOnly = /^\/p\/([^/]+)\/?$/.exec(location.pathname);
    if (pubOnly) return;
  }
 ...
 */
function createPageShell(pageNum, targetWidth, targetHeight) {
...
function setReaderLoadingCover(publication) {
  ...
  clearReaderLoadingCover();
  const url = String(
    publication?.cover_thumb_url || publication?.cover_url || ''
  ).trim();
  if (!url) return;
  ...
  img.decoding = 'async';
  img.className =
    'absolute inset-0 w-full h-full object-contain opacity-25 pointer-events-none select-none';
...
function bindReaderChromeControlsOnce() {
  ...
  document.addEventListener('click', (e) => {
    ...
    const actionEl = t.closest(
      '#reader-close, #reader-first, #reader-prev, #reader-next, #reader-last, #reader-zoom-in, #reader-zoom-out, #reader-fit-reset, #reader-fullscreen, #reader-page-jump-go, #reader-download-link'
    );
    if (!actionEl) return;
...
async function buildFlipFromPdfDoc(pdfDoc, myLoad) {
  ...
  for (let i = 1; i <= numPages; i++) {
    const shell = createPageShell(i, pageWidth, pageHeight);
    shells.push(shell);
    flipbookContainer.appendChild(shell);
  }

  const firstSpreadEnd = Math.min(
    typeof window !== 'undefined' && window.innerWidth >= 768 ? 4 : 2,
    numPages
  );
  if (readerLoadingDetail) {
    readerLoadingDetail.textContent =
      firstSpreadEnd > 1 ? 'Rendering first pages…' : 'Rendering cover…';
  }
  if (readerProgress) readerProgress.style.width = '100%';

  for (let i = 1; i <= firstSpreadEnd; i++) {
    if (myLoad !== loadGeneration) return;
    await renderPageIntoShell(pdfDoc, i, shells[i - 1], pageWidth, pageHeight);
  }
  ...
  const ready = new Set();
  for (let i = 1; i <= firstSpreadEnd; i++) ready.add(i);
  const pending = [];
  for (let i = firstSpreadEnd + 1; i <= numPages; i++) pending.push(i);
...
function updatePageInfo() {
  ...
  if (readerPageInfo) {
    ...
    readerPageInfo.textContent = `${oneBased} / ${total}`;
...
export function openReader(publication) {
  ...
  // Pointer/zoom/download binds are element-scoped: re-run each open so a
  // remounted reader chrome rebinds to its fresh DOM nodes.
  bindReaderPointerGesturesOnce();
  // Overlay (studio) still uses a dedicated download listener; page mode uses chrome delegation.
  if (!isReaderPageMode()) bindReaderDownloadOnce();
