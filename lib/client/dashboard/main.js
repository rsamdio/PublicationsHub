...
import { renderFirstPageWebpFromPdfFile } from '@/lib/client/pdf-first-page-webp.js';
import {
  openReader,
  closeReader,
  flipPrev,
...
function renderStudioFromLiveData(publisherId, series, editions, invites, roster) {
        ...
        card.innerHTML = `
          ...
          <div class="p-5 flex-1 flex flex-col">
            ...
            <div class="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
              <button type="button" class="series-btn-copy text-xs font-semibold px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-primary/10 hover:text-primary transition-colors inline-flex items-center gap-1" data-series-id="${escapeHtml(s.id)}" title="Copy publication link">
                <span class="material-icons text-[14px]">content_copy</span>Copy link
              </button>
  ...
  if (editionsGrid) {
    ...
    if (totalVol === 0 && contentFlowStep === 'editions' && selectedContentSeriesId) {
    ...
    editionsForGrid.forEach((ed, idx) => {
      ...
      editBtn.type = 'button';
      editBtn.className =
        'absolute top-2 right-2 z-10 flex h-9 w-9 items-center justify-center rounded-lg bg-black/55 text-white hover:bg-black/75 backdrop-blur-sm border border-white/15 shadow-sm transition-colors';
      ...
      copyBtn.type = 'button';
      copyBtn.className =
        'absolute top-2 right-12 z-10 flex h-9 w-9 items-center justify-center rounded-lg bg-black/55 text-white hover:bg-black/75 backdrop-blur-sm border border-white/15 shadow-sm transition-colors';
      copyBtn.title = 'Copy public link';
      copyBtn.setAttribute('aria-label', 'Copy public link');
      copyBtn.innerHTML = '<span class="material-icons text-base">content_copy</span>';
      ...
      card.addEventListener('pointerenter', () => {
        if (ed.pdf_url) warmReaderForEdition(ed.pdf_url);
        else void preloadReaderAssets();
      });
