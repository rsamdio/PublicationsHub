/**
 * Publisher/editor Firestore writes + RTDB reads (/studio).
 */
import {
  collection,
  doc,
...
import { fbAuth, fbDb, fbRtdb, fbFunctions } from './init';
...
export async function createSeries({ publisherId, title, description, frequency, slug: customSlug }) {
  try {
    const auth = fbAuth();
    const uid = auth.currentUser?.uid;
    if (!uid) return { data: null, error: { message: 'Not signed in' } };
    const slug = customSlug && String(customSlug).trim()
      ? String(customSlug).trim()
      : (title || '')
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, '-')
          .replace(/^-|-$/g, '');
